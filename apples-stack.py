...
seq = [2, 2, 1, 2, 1, 2, 1, 1, 2, 2, 3, 3, 1, 3, 5, 3, 4, 4, 2, 5, 3, 5, 2, 3, 4, 5, 5,4,5,5,4,4,4,4,5,5,5,4,4]
#seq = [2, 1, 2, 3, 3, 1, 3, 5]
...
def calc_largest(cur_largest,stack,cur):
    s = sum(i['l'] for i in stack) + cur['l']
    print("run",s, " largest:",cur_largest['length'])
    if s > cur_largest['length']:
        largest = {
...
def solution2():
    ...
    for n in sums:  
        ...
        if n['a'] == cur['a'] or n['a'] == stack[-1]['a']: # we continue the run
            ...
            largest = calc_largest(largest,stack,cur)
